...
const getAll = async (req, res) => {
  const result = await mongodb.getDatabase().collection('shark_attacks').find();
...
const getSingle = async (req, res) => {
  ...
  const result = await mongodb.getDatabase().collection('shark_attacks').findOne({ _id: attackId });
  if (!result) {
...
const createAttack = async (req, res) => {
  ...
  const response = await mongodb.getDatabase().collection('shark_attacks').insertOne(attack);
...
const updateAttack = async (req, res) => {
  ...
  const response = await mongodb.getDatabase().collection('shark_attacks').replaceOne({ _id: attackId }, attack);
...
const deleteAttack = async (req, res) => {
  const attackId = new ObjectId(req.params.id);
  const response = await mongodb.getDatabase().collection('shark_attacks').deleteOne({ _id: attackId });
